...
int main(int argc, char** argv)
{
    ...
    ros::Publisher vel_rviz_pub = nh.advertise<std_msgs::Float32>("twist_maneger/vel_rviz", 10);
    ros::Publisher yawVel_rviz_pub = nh.advertise<std_msgs::Float32>("twist_maneger/yawVel_rviz", 10);
        ...
        if(path.poses.size()>0){
            if(run_init){
                ...
                if(mode.data == "run"){
                    ...
                    cmd_vel.linear.x = 0;
                    cmd_vel.angular.z = constrain(diffAngle * 2, -maxYaw_rate, maxYaw_rate);
                    if(abs(diffAngle) < 3*M_PI/180){
                        run_init = false;
                    }
                }
            }
